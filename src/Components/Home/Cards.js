...
export default function Cards() {
    ...
    // Fetch courses from both API endpoints
    useEffect(() => {
    ...
    return (
        <div>
            ...
            <div className="container my-5 p-3">
                ...
                <div className="row">
                    {mainCourses.map(courses => (
                        <div key={courses.id} className="col-md-3 col-sm-6 mb-4">
                            <Link to={`/courses/${courses.id}`} className="text-decoration-none">
                                <div className="card">
                                    <img src={courses.imgSrc || "/default_course_image.jpg"} className="card-img-top" alt={courses.title} />
                                    <div className="card-body d-flex flex-column">
                                        <div className="course-logos">
                                            {Educator[courses.educator] && <img src={Educator[courses.educator]} alt="Course Logo" />}
                                        </div>
                                        <h5 className="card-title" style={{color: 'Black'}}>{courses.title}</h5>
                ...
                {showAdditionalCourses && (
                    <div className="row mt-4 fade-in">
                        {additionalCourses.map(courses => (
                            <div key={courses.id} className="col-md-3 col-sm-6 mb-3">
                                <Link to={`/courses/${courses.id}`} className="text-decoration-none">
                                    <div className="card">
                                        <img src={courses.imgSrc || "/default_course_image.jpg"} className="card-img-top" alt={courses.title} />
                                        <div className="card-body d-flex flex-column">
            ...
            <style>{`
                ...
                .card { transition: transform 0.3s ease, box-shadow 0.3s ease; padding: 7px; border-radius: 7px; height: 100%; }
                .card:hover { transform: scale(1.05); box-shadow: 0 7px 14px rgba(0, 0, 0, 0.2); }
                .card-img-top { height: 60%; border-radius: 10px; object-fit: cover; }
                .course-logos { display: flex; margin-bottom: 8px; justify-content: left; }
                .course-logos img { height: 24px; width: auto; margin-right: 8px; }
                .card-title { font-size: 16px; font-weight: 600; }
                .card-text { font-size: 0.8rem; }
                .button-container { display: flex; justify-content: flex-start; margin-top: 20px; }
                .fade-in { animation: fadeIn 0.5s ease-out; }
                @media (max-width: 576px) {
                    .course-logos img { height: 20px; }
